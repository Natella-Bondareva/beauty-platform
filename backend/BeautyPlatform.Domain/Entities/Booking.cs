...
public class Booking
{
    ...
    public ICollection<BookingFieldAnswer> FieldAnswers { get; private set; } = new List<BookingFieldAnswer>();
    ...
    /// <summary>
    /// Фабричний метод для створення Pending-бронювання з явним expiresAt.
    /// Не перевіряє правило "мінімум 15 хвилин в майбутньому" — використовується для тестів.
    /// </summary>
        ...
        DateTime? expiresAt = null)
    {
        ...
        b.StartTimeUtc = startTimeUtc;
        b.EndTimeUtc   = endTimeUtc;
        b.Price        = price;
        b.Status       = BookingStatus.Pending;
        b.ExpiresAt    = expiresAt ?? DateTime.UtcNow.Add(PendingTtl);
        b.CreatedAt    = DateTime.UtcNow;
        return b;
    }

    /// <summary>
    /// Фабричний метод для запису адміном:
    /// — не перевіряє правило "мінімум 15 хвилин в майбутньому"
    /// — одразу встановлює статус Confirmed (без SMS)
    /// </summary>
    public static Booking CreateByAdmin(
        Guid salonId,
        Guid clientId,
        Guid employeeId,
        Guid serviceId,
        ...
        decimal price)
    {
        ...
        b.ExpiresAt    = DateTime.UtcNow; // не релевантно для адмін-запису
        b.CreatedAt    = DateTime.UtcNow;
        return b;
    }
}
